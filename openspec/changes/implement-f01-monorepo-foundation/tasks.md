...
- [ ] 1.4 Keep `packages/tuyau` as a minimal integration boundary without domain endpoint calls or frontend wiring.
...
- [ ] 2.3 Ensure any package affected by root Turbo tasks has the scripts needed to participate or a deliberate no-op script when implementation is not available yet.
...
- [ ] 4.2 Run root workspace validation commands relevant to F01, including at least `pnpm typecheck` and `pnpm build` if available.
